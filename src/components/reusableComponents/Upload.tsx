...
const Upload = (props: UploadImageProps) => {
    ...
    return (
      ...
      <div
        ...
        className="flex cursor-pointer w-[120px] h-[120px] rounded-full border-[2px] border-[#F23F39] border-solid justify-center items-center "
      >
        {imageSrc ? (
          <img src={imageSrc} alt="Uploaded" className="w-full h-full rounded-full object-cover" />
        ) : (
          <img src={props.user ? uploadUser : uploadImg} alt="" />
        )}
      </div>
      <button
        type="button"
        onClick={handleButtonClick}
        className="flex justify-between items-center underline gap-[2px] text-[#F23F39]"
      >
        <h5>Add Photo</h5>
        <svg
          width="12"
